...
### Acceptance criteria
- [x] `internal/network.Lookup(Mainnet)` returns
...
- [x] The sentinel `ErrMainnetNotEnabled` is removed or — if other packages
      depend on it — re-purposed and documented as deprecated.
- [x] A dedicated unit test asserts the mainnet fork bytes byte-for-byte:
      `[4]byte{0x00,0x00,0x00,0x00}`.
- [x] `ParseFlag("mainnet")` returns `Mainnet, nil`.
- [x] Existing Hoodi tests still pass unchanged.
...
### Acceptance criteria
- [x] `testdata/mainnet/` is committed with `keystore.json`,
      `passphrase.txt`, `pubkeys.txt`, and `deposit_data-expected.json`.
      **Note:** fixtures are self-referential (generated by this Go implementation,
      not `staking-deposit-cli`) because the CLI only accepts mnemonic-derived keys;
      deviation documented in `docs/validation/mainnet-golden.md`. Cross-validation
      against `staking-deposit-cli` is deferred to Phase 3.
- [x] A test mirroring the Hoodi golden test (`test/e2e/mainnet_test.go`)
      runs the full pipeline against these fixtures and
      asserts field-by-field equivalence with `deposit_data-expected.json`.
- [x] The test verifies the emitted `fork_version` is `"00000000"` and
      `network_name` is `"mainnet"`.
- [x] A test asserts that running with `--network mainnet` causes the
      Phase-1 confirmation banner to include the literal string `mainnet`
      on stderr before any signing happens.
- [x] `make refresh-golden` is extended to regenerate both `hoodi/` and
      ...
      regeneration date.
- [x] CI runs both Hoodi and mainnet golden tests in the default
      `go test ./...` invocation.
...
- [ ] `docs/validation/hoodi-e2e.md` is committed and records: validator
      ...
      emitted. **Status: PENDING** — template committed; awaiting operator run.
...
### Acceptance criteria
- [x] When `--network mainnet` is selected, the app **requires** the
...
- [x] For `--network hoodi`, the new flag is accepted but has no effect
      (no spurious warning).
- [x] The banner printed before signing on mainnet contains the literal
      string `MAINNET` (uppercased) in addition to the Phase-1 fields.
- [x] Unit tests cover: mainnet without ack flag → exit code 2 and no
      ...
      normally.
- [x] Help text documents the flag with a one-line warning about its
      irreversible consequence.
- [x] The behaviour is mentioned in `docs/validation/mainnet-golden.md`
